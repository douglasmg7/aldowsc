...
# If today file not downloaded or have a invalid size.
EXIST_TODAY_XML_FILE=$(ls -lh $ZUNKAPATH/xml/aldo/ | grep -E aldo-products-$(date +%Y-%h-%d)-[0-9]{6}\.xml | cut -d " " -f5 | sed "s/[A-Z,a-z]//" | awk '$1>100')
if [[ -z $EXIST_TODAY_XML_FILE ]]; then
    # Get xml file.
...
if [[ $RUN_MODE == production ]]; then
    RUN_MODE=production aldowsc --update-stock-quantity 
else
    go build $GS/aldowsc
    $GS/aldowsc/aldowsc --update-stock-quantity
fi